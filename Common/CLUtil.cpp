...
size_t CLUtil::GetGlobalWorkSize(size_t DataElemCount, size_t LocalWorkSize)
{
    ...
    // 
    //determines the Global worksize based on the number of data elements and the local work size
    //example: 
    //(DataElemCount = 4, LocalWorkSize = 2) -> GlobalWorkSize = 4
    //(DataElemCount = 5, LocalWorkSize = 2) -> GlobalWorkSize = 6 (3 Work-Groups of size 2 needed)
...
cl_program CLUtil::BuildCLProgramFromMemory(cl_device_id Device, cl_context Context, const std::string& SourceCode, const std::string& CompileOptions)
{
    ...
    cl_int clError;
    //create one program object using the given source
    ...
    PrintBuildLog(prog, Device); //lists the related error messages to the console
    if (CL_SUCCESS != clError)
    {
		...
		const size_t* pGlobalWorkSize, const size_t* pLocalWorkSize, int NIterations)
{
    ...
    //measuring the ellapsed time for N executions of the kernel and dividing the result by N
