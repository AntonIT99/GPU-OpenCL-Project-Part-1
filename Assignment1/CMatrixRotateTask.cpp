...
bool CMatrixRotateTask::InitResources(cl_device_id Device, cl_context Context)
{
	...
	V_RETURN_FALSE_CL(clError, "Failed to create the buffer M");
	m_dMR = clCreateBuffer(Context, CL_MEM_WRITE_ONLY, sizeof(cl_float) * m_SizeX * m_SizeY, NULL, &clError);
	V_RETURN_FALSE_CL(clError, "Failed to create the buffer MR");

	size_t programSize = 0;
	string programCode;

	//create program object
	//Load the program code from the given location: path of the kernel MatrixRot
	...
	V_RETURN_FALSE_CL(clError, "Failed to set kernel arguments: MatrixRotNaive");
	...
	V_RETURN_FALSE_CL(clError, "Failed to set kernel arguments: MatrixRotOptimized");
...
void CMatrixRotateTask::ComputeGPU(cl_context Context, cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	time = CLUtil::ProfileKernel(CommandQueue, m_NaiveKernel, 2, globalWorkSize, LocalWorkSize, Nexecutions);
	...
	V_RETURN_CL(clErr, "Error copying output data for MatrixRotNaive");

	//optimized kernel
	// TO DO: allocate shared (local) memory for the kernel
	//bind argument for shared local memory
	cl_int clErrorSM;
	clErrorSM = clSetKernelArg(m_OptimizedKernel, 4, LocalWorkSize[0] * LocalWorkSize[1] * sizeof(float), NULL);
	V_RETURN_CL(clErrorSM, "Error allocating shared memory");
	// run kernel
	// TO DO: time = GLUtil::ProfileKernel...
	time = CLUtil::ProfileKernel(CommandQueue, m_OptimizedKernel, 1, globalWorkSize, LocalWorkSize, Nexecutions);
	...
	V_RETURN_CL(clErr, "Error copying output data for MatrixRotOptimized");
}
...
void printMatrix(float M[], int size_x, int size_y)
{
...
{
	/*printMatrix(m_hM, m_SizeX, m_SizeY);
	printMatrix(m_hMR, m_SizeY, m_SizeX);
	printMatrix(m_hGPUResultNaive, m_SizeY, m_SizeX);
	printMatrix(m_hGPUResultOpt, m_SizeY, m_SizeX);*/
